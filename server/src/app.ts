...
app.post(
  '/api/auth/login',
  rateLimit({ windowMs: 15 * 60_000, limit: 20, skip: () => config.isTest }),
  async (req, res) => {
    ...
    if (result.status === 'locked') {
      ...
      res.status(423).set('Retry-After', String(retryAfterSeconds)).json({
        error: {
...
app.post(
  '/api/auth/signup',
  rateLimit({ windowMs: 60 * 60_000, limit: 10, skip: () => config.isTest }),
  async (req, res) => {
